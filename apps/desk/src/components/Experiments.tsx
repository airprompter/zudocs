...
export function Experiments({ arms }: { arms: Arms | null }) {
  ...
  const inconsistent = arms.stickiness.filter((s) => !s.consistent);
  return (
    <section className="experiments" title={TOOLTIPS.arm}>
      ...
      {arms.ramps.map((r) => (
        <p key={r.experimentId} className="fine">
          ...
          <span className="muted"> · plan {r.plan.map((p, i) => `${i === r.step ? "▶ " : ""}${pct(p.weightBps[1] ?? p.weightBps[0] ?? 0)}`).join(" → ")}{r.nextStepAt ? ` · next step ${ago(r.nextStepAt).replace(" ago", "")}` : ""} · one approval unlocks the whole plan on eu-west</span>
        </p>
      ))}
      {live.length ? (
      ...
      {arms.stickiness.length ? (
        <p className="fine">
          <strong>Sticky by customer:</strong> {arms.stickiness.length} customer·experiment pairs across {new Set(arms.stickiness.flatMap((s) => Object.keys(s.arms))).size} host(s) — {inconsistent.length === 0 ? "every host agrees" : `${inconsistent.length} DISAGREE`}.
