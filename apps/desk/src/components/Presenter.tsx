 ...
 * host's queue), and the wire: cut (the eu-west host loses AirPrompter and Bedrock, keeps the desk's tables; a rule
 * restores it in 15 minutes whatever happens) and restore. Every button is an API call; the result lands as a
...
export function Presenter({ state, busy, selectedTicketId, onAction, environment, agentId }: { state: State | null; busy: string | null; selectedTicketId: string | null; onAction: (action: string, body?: Record<string, unknown>) => void; environment: string; agentId: string }) {
  ...
  const others = (state?.hosts ?? []).filter((h) => h.kind !== "lambda").map((h) => h.hostId);
