...
function describe(e: TimelineEvent): string {
  switch (e.kind) {
    ...
    case "power": return e.refusal ? `${e.action === "sleep" ? "sleep" : e.action === "wake" ? "wake" : "power"} refused on ${e.forHost}: ${e.refusal} (${e.by})` : e.observed ? `${e.forHost} found ${e.state} by ${e.by}` : `${e.forHost} ${e.state === "stopping" ? "going to sleep" : e.state === "pending" ? "waking" : String(e.state)}${e.instanceId ? ` (${e.instanceId})` : ""} by ${e.by}`;
