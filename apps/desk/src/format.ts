...
export const TOOLTIPS = {
  ...
  frozen: "A signed disable directive on the manifest: a host that syncs stops rendering the moment the manifest verifies. On the daemon host the workers render from the active release, so the frozen generation takes effect there when it is approved (an SDK gap, filed). Only the console lifts it.",
