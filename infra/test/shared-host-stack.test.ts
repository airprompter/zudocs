...
import { SCHEDULE_BY } from "../../services/eu-host/src/power.js";
import { EU_HOST_ROLE_NAME, POWER_FUNCTION_NAME, SLEEP_CRON_UTC, SLEEP_SCHEDULE_NAME, WIRE_FUNCTION_NAME, readPins } from "../lib/shared-host-names.js";
...
test("the instance role: Session Manager by its own actions (not the managed policy that reads every parameter), one parameter, the bundle, the models in the desk's region, the eight tables by ARN, its log group", () => {
  ...
  const parameter = statements.filter((st) => actionsOf(st).includes("ssm:GetParameter") && st.Sid !== "PowerDemoMode");
  assert.equal(parameter.length, 1, "the host's one parameter statement (the power function's demo-mode read is the other)");
  assert.ok(JSON.stringify(parameter[0]!.Resource).includes(":ssm:eu-west-1:111122223333:parameter/zudocs/dev/agent-key"), "the Agent key, in this region, by ARN");
  assert.ok(JSON.stringify(parameter[0]!.Resource).includes(":ssm:eu-west-1:111122223333:parameter/zudocs/dev/demo-mode"), "and the demo-mode switch (phase 8), by ARN");
  assert.equal((JSON.stringify(parameter[0]!.Resource).match(/parameter\//g) ?? []).length, 2, "two parameters, nothing wider");
  const dynamo = statements.filter((st) => actionsOf(st).some((a) => a.startsWith("dynamodb:")) && actionsOf(st).length > 1 && st.Sid !== "PowerMarker");
  assert.equal(dynamo.length, 1, "the host's one table statement (the wire's PutItem and the power function's marker are the others)");
  ...
  assert.equal(logsStatements.length, 1, "the host's own group (the functions log under the basic execution policy)");
...
test("the wire function: Node 22 arm64 by its fixed name, the group id and the limit in its environment, write access to its own group and the events table only, a five-minute tick", () => {
  ...
  const describe = statements.find((st) => actionsOf(st).includes("ec2:DescribeSecurityGroups") && st.Sid !== "PowerDescribe")!;
  ...
  assert.equal(put.length, 2, "the wire and the power function each write the timeline and nothing else there");
...
test("phase 8: the power function starts and stops only instances carrying the host's Name tag, reads one parameter, writes the marker and the timeline; the nightly sleep is a Scheduler schedule with no start; the demo-mode parameter is created off", () => {
  const { sharedHost } = synthAll();
  sharedHost.hasResourceProperties("AWS::Lambda::Function", { FunctionName: POWER_FUNCTION_NAME, Runtime: "nodejs22.x", Architectures: ["arm64"], Handler: "index.handler", Timeout: 30, Environment: { Variables: Match.objectLike({ NAME_TAG: "zudocs-eu-host", HOST_ID: "eu-west-1/ec2", DYNAMODB_REGION: "us-east-1", STATUS_TABLE: "zudocs-desk-status", EVENTS_TABLE: "zudocs-desk-events", DEMO_MODE_PARAMETER: "/zudocs/dev/demo-mode" }) } });
  const statements = statementsOf(sharedHost);
  const startStop = statements.find((st) => st.Sid === "PowerStartStop")!;
  assert.deepEqual(actionsOf(startStop).sort(), ["ec2:StartInstances", "ec2:StopInstances"]);
  assert.equal(startStop.Resource, "arn:aws:ec2:eu-west-1:111122223333:instance/*");
  assert.deepEqual(startStop.Condition, { StringEquals: { "ec2:ResourceTag/Name": "zudocs-eu-host" } }, "only an instance that carries the host's Name tag");
  assert.equal(statements.filter((st) => actionsOf(st).some((a) => /ec2:(Start|Stop|Terminate|Reboot|Modify|Run)Instances/.test(a))).length, 1, "no other statement controls an instance; nothing terminates, reboots or launches");
  const describe = statements.find((st) => st.Sid === "PowerDescribe")!;
  assert.deepEqual(actionsOf(describe).sort(), ["ec2:DescribeInstances", "ec2:DescribeSecurityGroups"], "read-only, to find the instance and to see the wire");
  const demo = statements.find((st) => st.Sid === "PowerDemoMode")!;
  assert.deepEqual(actionsOf(demo), ["ssm:GetParameter"]);
  assert.equal(demo.Resource, "arn:aws:ssm:eu-west-1:111122223333:parameter/zudocs/dev/demo-mode", "the switch only — never the Agent key");
  const marker = statements.find((st) => st.Sid === "PowerMarker")!;
  assert.deepEqual(actionsOf(marker).sort(), ["dynamodb:GetItem", "dynamodb:UpdateItem"]);
  assert.equal(marker.Resource, "arn:aws:dynamodb:us-east-1:111122223333:table/zudocs-desk-status", "the status row's marker, and no delete");
  const timeline = statements.find((st) => st.Sid === "PowerTimeline")!;
  ...
  const sleep = schedules[0]!.Properties;
  assert.equal(sleep.Name, SLEEP_SCHEDULE_NAME);
  assert.equal(sleep.ScheduleExpression, SLEEP_CRON_UTC);
  assert.equal(sleep.ScheduleExpression, "cron(0,20,40 10 * * ? *)", "three attempts twenty minutes apart at ten in the morning UTC");
  ...
  sharedHost.hasResourceProperties("AWS::SSM::Parameter", { Name: "/zudocs/dev/demo-mode", Type: "String", Value: Match.stringLikeRegexp('"mode":"off"') });
  assert.equal(Object.values(sharedHost.findResources("AWS::SSM::Parameter") as Resources).filter((p) => p.Properties.Type === "SecureString").length, 0, "no SecureString is ever templated: the Agent key is the owner's, by hand");
  for (const output of ["PowerFunctionName", "DemoModeParameterName", "SleepSchedule"]) sharedHost.hasOutput(output, {});
});
