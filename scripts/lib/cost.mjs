 ...
 * document, the expected month at current usage — and the numbers section `the owner's cost document` carries between its
 ...
 * spliceNumbers(docText, renderNumbersSection({ last7, last30, budget, now }));   // the owner's cost document with its numbers replaced
 * ```
 */
...
/** The markers `the owner's cost document` keeps its generated numbers between. */
...
/** The numbers section for the owner's cost document, markers included. */
...
export function spliceNumbers(docText, section) {
  ...
  if (start < 0 || end < 0 || end < start) throw new Error(`the owner's cost document: the markers ${NUMBERS_START} … ${NUMBERS_END} are missing or out of order`);
