...
/** Per slot: the call site's values, the customer the render is for, whether it must carry success criteria, a canned answer. */
export const SCENARIOS = {
  ...
  "support.reply": { values: { ticket: TICKET }, subject: "cust-2002", criteria: true, answer: "Thanks for flagging this — a deleted page lingering in search is our index running behind. I have queued a re-index of your space; results usually catch up within the hour. Your trial includes full search, so nothing to change on your side. If it is still there tomorrow, send me the page URL and I will look directly.\n\nThe Zudocs team" },
...
export function substitutionProof(a, valueA, b, valueB) {
  const occurrences = a.split(valueA).length - 1;
  if (occurrences === 0) return { ok: false, reason: "absent", occurrences };
  if (a.replaceAll(valueA, valueB) !== b) return { ok: false, reason: "differs_elsewhere", occurrences };
