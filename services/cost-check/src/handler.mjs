/**
 * The monthly cost check: on the first of the month (EventBridge Scheduler, `site-stack.ts`) — or by hand with
 ...
 * can watch the line. Two Cost Explorer calls a month ($0.02). No secrets: nothing here is one.
 *
 * @example
 * ```sh
...
import { METRIC_NAMESPACE, budgetOf, costQuery, costWindow, expectedMonthlyUsd, monthWindow, monthlyDocument, previousMonth, summarise } from "../../../scripts/lib/cost.mjs";
...
export async function costCheck(event, ports) {
  ...
  const [monthResults, weekResults, budgetOut] = await Promise.all([ports.costAndUsage(costQuery(effective)), ports.costAndUsage(costQuery(costWindow(now, 7))), ports.describeBudget(ports.budgetName)]);
  const monthSummary = summarise(monthResults, effective);
  const last7 = summarise(weekResults, costWindow(now, 7));
  ...
  return { month, partial, key, monthTotalUsd: monthSummary.total, expectedMonthlyUsd: expectedMonthlyUsd(last7), budgetActualUsd: budget?.actualUsd ?? null, metrics: metrics.map((m) => m.MetricName) };
}
...
export const handler = async (event) => {
  ...
  const answer = await costCheck(event ?? {}, {
    ...
    costAndUsage: async (query) => (await ce.send(new GetCostAndUsageCommand(query))).ResultsByTime ?? [],
